...
public class TseitinTransformation{
    ...
    public static void main(String[] args) {
        ...
        list.add(test);
        Constraint end = tseitinTransformation(list, problem);
        //System.out.println (end);
    }


    public static Constraint tseitinTransformation (ArrayList<Constraint> args, SmtProblem problem){
        System.out.println ("converting: " + args.get(0));
        if (inCNF(args)) return SmtFactory.createConjunction(args);
        ...
        ArrayList<Constraint> biImplications = makeBiImplications(problem, SmtFactory.createConjunction(args));
        ...
        //     BVar newVar = SmtFactory.createBooleanVariable(problem);
        //     auxVariables.add(newVar);
        // }
        
        // for (int i =subFormulas.size()-1; i >= 0; i--){
        //     convert(subFormulas.get(i), problem);
        // }
        
        // ArrayList<Constraint> replacedSubformulas = new ArrayList<>();
        // System.out.println ("aux vars: " + auxVars);
        // for (Constraint key : auxVars.keySet()){
        //     System.out.println (key + " converted is " + replace(key));
        //     replacedSubformulas.add(replace(key));
        // }
        // System.out.println (auxVars.keySet());
        // System.out.println ("replaced sub formulas: " + replacedSubformulas);
        // ArrayList<Constraint> listWithoutDuplicates = new ArrayList<>(new LinkedHashSet<>(replacedSubformulas));
        // System.out.println ("replaced sub formulas without duplicates: " + listWithoutDuplicates);
        // //initiate all to false/empty constraints
        // for (int i =0; i < subFormulas.size(); i++) replacedSubformulas.add(SmtFactory.createFalse());
        // for (int i = 0; i < auxVariables.size(); i++){
        //     System.out.println ("replacing " + subFormulas.get(i) + " for " + auxVariables.get(i));
        //     for (int j =0; j < subFormulas.size(); j++){
        //         if (i != j) {
        //             Constraint result = replace(subFormulas.get(j), subFormulas.get(i), auxVariables.get(i));
        //             //System.out.println ("replacing " + subFormulas.get(i)  + " for " + auxVariables.get(i)+ " in " + subFormulas.get(j));
        //             System.out.println ("result is " + result);
        //             if ()
        //             replacedSubformulas.add(result);
        //             //if (!(result.equals(subFormulas.get(j)))) replacedSubformulas.add(result);
        //         }
        //     }
        // }
        // System.out.println (replacedSubformulas);
        // ArrayList<Constraint> list = new ArrayList<>();
        // for (int i =0; i < replacedSubformulas.size(); i++){
        //     list.add(SmtFactory.createIff(auxVariables.get(i), replacedSubformulas.get(i)));
        // }
        // //ArrayList<Constraint> implications = makeImplications(subFormulas);
        // Constraint conjunction = SmtFactory.createConjunction(replacedSubformulas);
        // System.out.println (list);
        // if (inCNF(conjunction)) return conjunction;
        // else {
        //     System.out.println ("not in cnf");
        //     return convertToCNF(conjunction);
        // }
   ...
   // }
        ...
        if (c instanceof Conjunction con){
            ...
            for (int i =1; i <= con.numChildren(); i++){
                if (con.queryChild(i) instanceof Disjunction d){
                    args.add(distributiveLaw(con.queryChild(i)));
                }
                else args.add(deMorgan(con.queryChild(i)));
            }
        ...
        if (c instanceof Disjunction d){
            for (int i =1; i < d.numChildren(); i++){
                ...
                if (inCNF(c)) return c;
                    ...
                    args.add(distributiveWithOneAnd(deMorgan(firstChild), con));
                }
                else if ((secondChild instanceof Not || secondChild instanceof BVar) && firstChild instanceof Conjunction con){
                    args.add( distributiveWithOneAnd (deMorgan(secondChild), con));
                }
                    ...
                    args.add(SmtFactory.createDisjunction(deMorgan(n), b));
                }
                else if (secondChild instanceof Not n && firstChild instanceof BVar b){
                    args.add(SmtFactory.createDisjunction(deMorgan(n), b));
                }
                else throw new Error (firstChild + " or " + secondChild + " not supported.");
            }
        }
        return SmtFactory.createConjunction(args);
    }
    ...
    public static ArrayList<Constraint> returnSubFormulas (Constraint c){
        ArrayList<Constraint> subFormulas = new ArrayList<>();
        switch (c){
