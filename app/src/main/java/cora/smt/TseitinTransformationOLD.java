...
public class TseitinTransformationOLD{
    ...
    //     final Constraint full = ((Iff)biImplications.get(0)).queryLeft();
    ...
    public static Constraint tseitinTransformation (Constraint formula, SmtProblem problem){
        ...
        for (int i =0; i < biImplications.size(); i++){
            if (!inCNF(biImplications.get(i))){
                throw new Error (biImplications.get(i) + " not in cnf");
            }
        }
        biImplications.add(full);
        System.out.println ("added: " + full);
    ...
    public static ArrayList<Constraint> makeSubFormulas (SmtProblem problem, Constraint c){
        ...
        switch (c){
            ...
            case Disjunction d: 
                ...
                break;
            case Not n:
                if (n.queryChild() instanceof BVar) return new ArrayList<>();
                subFormulas.add(n);
   ...
   // }
    ...
    public static Constraint deMorgan (Constraint c){
        //System.out.println ("in demorgan for " + c);
        if (inCNF(c)) return c;
        else if (c instanceof Disjunction){
            
            return distributiveLaw(c);
        }
        else if (c instanceof Conjunction con){
            ArrayList<Constraint> args = new ArrayList<>();
            for (int i =1; i <= con.numChildren(); i++){
                //if (inCNF(con.queryChild(i))) args.add(con.queryChild(i));
            ...
            if (child instanceof Conjunction con){
                ...
                return SmtFactory.createDisjunction(args).simplify();
            }
    ...
    public static Constraint distributiveLaw (Constraint c){
        ...
        if (c instanceof Disjunction d){
            for (int i =1; i < d.numChildren(); i++){
                Constraint firstChild = SmtFactory.createFalse();
                Constraint secondChild = SmtFactory.createFalse();
                if (i==1){
                    firstChild = d.queryChild(i);
                    secondChild = d.queryChild(i+1);
                }
                else {
                    firstChild = args.get(args.size()-1);
                    secondChild = d.queryChild(i+1);

                }
                System.out.println ("first child instanceof " + firstChild.getClass()+ " and second child " + secondChild.getClass());

                if (firstChild instanceof BVar && secondChild instanceof BVar){
                    args.add(firstChild);
                    args.add(secondChild);
                }
                else if ((firstChild instanceof Not|| firstChild instanceof BVar || firstChild instanceof Disjunction)&& secondChild instanceof Conjunction con){
                    args.add(distributiveWithOneAnd(deMorgan(firstChild), con));
                }
                else if ((secondChild instanceof Not || secondChild instanceof BVar || secondChild instanceof Disjunction) && firstChild instanceof Conjunction con){
                    args.add( distributiveWithOneAnd (deMorgan(secondChild), con));
                    System.out.println ("in here");
                    System.out.println ("returning: " + distributiveWithOneAnd (deMorgan(secondChild), con));
                }
                    ...
                    args.add(SmtFactory.createDisjunction(deMorgan(n.simplify()), b).simplify());
                }
                else if (secondChild instanceof Not n && firstChild instanceof BVar b){
                    args.add(SmtFactory.createDisjunction(deMorgan(n.simplify()), b).simplify());
                }
                else if (firstChild instanceof Not n1 && secondChild instanceof Not n2){
                    args.add(SmtFactory.createDisjunction(deMorgan(n1.simplify()), deMorgan(n2.simplify())).simplify());
                }
                else if (firstChild instanceof Disjunction){
                    firstChild = distributiveLaw(firstChild);
                    args.add(firstChild);
                    if (!(secondChild instanceof Disjunction)) args.add(secondChild);
                }
                else if (secondChild instanceof Disjunction){
                    secondChild = distributiveLaw(secondChild);
                    args.add(secondChild);
                    if (!(firstChild instanceof Disjunction)) args.add(firstChild);
                }
                // else if (inCNF(SmtFactory.createDisjunction(firstChild, secondChild)))
                else if (!(inCNF(firstChild) || !(inCNF(secondChild)))) throw new Error (firstChild + " or " + secondChild + " not supported in dislaw.");
    ...
    public static Constraint distributiveWithOneAnd (Constraint n, Conjunction c){
        ArrayList<Constraint> args = new ArrayList<>();
        for (int i =1; i <= c.numChildren(); i++){ 
            args.add( SmtFactory.createDisjunction(n, c.queryChild(i)).simplify());
        }
        return SmtFactory.createConjunction(args).simplify();
    }
    ...
    public static Constraint distributiveWithTwoAnds (Conjunction c1, Conjunction c2){
        ...
        for (int i =1; i <= c1.numChildren(); i++){
            for (int j=1; j<= c2.numChildren(); j++){
                args.add(SmtFactory.createDisjunction(c1.queryChild(i), c2.queryChild(j)).simplify());
            }
        }
        return SmtFactory.createConjunction(args).simplify();
    }
